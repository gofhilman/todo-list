...
function handleTaskDetails(
  ...
  if (mainList.includes(projectObj)) {
    const taskCreationSection = document.querySelector(
...
function handleTaskList(event, projectObj) {
  if (event.target.tagName === "P") {
    let taskObj = projectObj.taskList.find(
      (task) => task.id === event.target.id
    );
    updateTaskDetails(projectObj, taskObj);
    if (mainList.includes(projectObj)) {
      const taskDeletion = document.querySelector("#task-deletion");
      taskDeletion.classList.add("hidden");
    }
  }
}

function handleProjectList(event) {
  let projectObj = projectList.find(
...
function handleMainList(event) {
  ...
  let mainObj = mainList.find(
    (main) => main.id === event.target.parentElement.id
  );
  updateMain(mainObj);
...
function handleListMark(event, projectObj) {
  if (
    event.target.className === "circle-mark" ||
    event.target.className === "star-mark"
  ) {
    let targetId, markFn;
...
function handleCompletedList(event, projectObj) {
  if (
    event.target.id === "completed-list-icon" ||
    event.target.id === "completed-list-text"
  ) {
